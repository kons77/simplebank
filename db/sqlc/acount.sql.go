...
const updateAccountNoReturn = `-- name: UpdateAccountNoReturn :exec
UPDATE accounts
SET balance = $2
WHERE id = $1
`

type UpdateAccountNoReturnParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) UpdateAccountNoReturn(ctx context.Context, arg UpdateAccountNoReturnParams) error {
	_, err := q.db.Exec(ctx, updateAccountNoReturn, arg.ID, arg.Balance)
	return err
}
